...
<body>
	...
	<form method="post">
		...
		%>
		<a>首页</a> <a>上一页</a> <a
			href="QueryInPageStudentsBySageServlet?currentPage=<%=p.getCurrentPage() + 1%>">下一页</a>
		<a
			href="QueryInPageStudentsBySageServlet?currentPage=<%=p.getTotalPage()%>">尾页</a>
		<%
			} else {
		%>
		...
		%>
		<a href="QueryInPageStudentsBySageServlet?currentPage=1">首页</a> <a
			href="QueryInPageStudentsBySageServlet?currentPage=<%=p.getCurrentPage() - 1%>">上一页</a>
		<a>下一页</a> <a>尾页</a>
		<%
			} else {//中间页
		%>
		<a href="QueryInPageStudentsBySageServlet?currentPage=<%=1%>">首页</a>
		<a
			href="QueryInPageStudentsBySageServlet?currentPage=<%=p.getCurrentPage() - 1%>">上一页</a>
		<a
			href="QueryInPageStudentsBySageServlet?currentPage=<%=p.getCurrentPage() + 1%>">下一页</a>
