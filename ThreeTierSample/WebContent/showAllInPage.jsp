...
<body>
	...
	<form method="post">
		...
		<a href="add.jsp">新增</a> <a href="QueryAllStudentsServlet">返回</a><br />
		<br />
		<%
			if (p.getCurrentPage() == 1) {//首页
				if (p.getTotalPage() > 1) {//总页数
		%>
		<a>首页</a> <a>上一页</a> <a
			href="QueryInPageAllStudentsServlet?currentPage=<%=p.getCurrentPage() + 1%>">下一页</a>
		<a
			href="QueryInPageAllStudentsServlet?currentPage=<%=p.getTotalPage()%>">尾页</a>
		<%
			} else {
		%>
		...
		%>
		<a href="QueryInPageAllStudentsServlet?currentPage=1">首页</a> <a
			href="QueryInPageAllStudentsServlet?currentPage=<%=p.getCurrentPage() - 1%>">上一页</a>
		<a>下一页</a> <a>尾页</a>
		<%
			} else {//中间页
		%>
